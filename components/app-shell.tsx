...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-4 pb-24 pt-3 sm:px-6 sm:pb-6 lg:px-8">
      <header className="sticky top-0 z-20 -mx-4 border-b border-slate-200/70 bg-white/80 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          {user ? (
            <button
              onClick={signOut}
              className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500 transition hover:text-slate-800 sm:px-4"
            >
              Sign out
            </button>
          ) : null}
      ...
      {showQuickAdd ? (
        <Link
          ...
          className="fixed bottom-24 right-4 z-30 grid h-12 w-12 place-items-center rounded-full bg-blue-600 text-2xl font-bold text-white shadow-[0_12px_30px_rgba(37,99,235,0.35)] sm:hidden"
        >
          +
        </Link>
      ) : null}
